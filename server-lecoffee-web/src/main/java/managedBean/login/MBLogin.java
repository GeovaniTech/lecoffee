...
public class MBLogin extends AbstractMBean {
	...
	public void logar() {
		if(EmailUtil.isValidEmailPattern(this.getEmail())) {
			if(this.getClientSBean().logar(this.getEmail(), this.getPassword())) {
				...
				switch (this.getClientSession().getSecurityLevel())	{
					case "admin":
					case "superadmin":
	...
	public void sendEmailForgotPassoword() {
		...
		description.append("<h2>Troca de Senha de acesso ao LeCoffee<h2/>");
		...
		description.append("<p>Caso você não tenha solicitado a troca de senha no LeCoffee ");
